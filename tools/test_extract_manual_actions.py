...
"""
Tests for extract_manual_actions.py
...
class TestGetLogbookEntries:
    ...
    @patch("extract_manual_actions.requests.get")
    def test_makes_correct_api_call(self, mock_get):
        """Should make correct API call with proper headers and params."""
        ...
        # Check URL contains start time
        assert "2025-01-20" in call_args[0][0]
    ...
    def test_returns_json_response(self, mock_get):
        """Should return JSON response from API."""
        expected_entries = [
            {"entity_id": "light.living_room", "state": "on"},
            {"entity_id": "switch.fan", "state": "off"},
        ]
        mock_response = MagicMock()
        mock_response.json.return_value = expected_entries
        mock_response.raise_for_status = MagicMock()
        mock_get.return_value = mock_response
    ...
    @patch("extract_manual_actions.requests.get")
    def test_includes_entity_filter_when_provided(self, mock_get):
        """Should include entity filter in params when provided."""
        mock_response = MagicMock()
        mock_response.json.return_value = []
        mock_response.raise_for_status = MagicMock()
        mock_get.return_value = mock_response
...
class TestGetHaToken:
    """Tests for the get_ha_token function."""

    @patch.dict("os.environ", {"HA_TOKEN": "env_token_123"})
    def test_returns_token_from_env_var(self):
        """Should return token from environment variable."""
        result = get_ha_token()
        assert result == "env_token_123"

    @patch.dict("os.environ", {}, clear=True)
    @patch("extract_manual_actions.Path")
    def test_returns_token_from_file(self, mock_path_class):
        ...
        mock_path.read_text.return_value = "file_token_456\n"
        mock_path_class.home.return_value.__truediv__.return_value = mock_path

        result = get_ha_token()
        assert result == "file_token_456"
